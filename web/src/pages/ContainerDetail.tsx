...
export default function ContainerDetail() {
  ...
  const handleSeal = async () => {
    ...
    try {
      await sealContainer(containerId, {
        ...
        temp_setpoint_c: sealForm.temp_setpoint_c ? Number(sealForm.temp_setpoint_c) : null,
      });
  ...
  const canDelete = container
    ? (container.status === "open" || container.status === "loading") && container.pallet_count === 0
    : false;
  ...
  const canRevert = ["loaded", "sealed", "dispatched", "in_transit", "arrived"].includes(container.status);
  ...
  return (
    <div className="max-w-4xl mx-auto px-6 py-8 space-y-6">
      ...
      <PageHeader
        ...
        action={
          <div className="flex items-center gap-2">
            <StatusBadge status={container.status} className="text-sm px-3 py-1" />
            {canDelete && (
              <button
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            <p className="text-sm text-yellow-800">
              {t("container.overdueWarning", {
                eta: container.eta ? new Date(container.eta).toLocaleDateString() : "?",
              })}
            </p>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <Card label={t("detail.weight")} value={container.gross_weight_kg ? `${container.gross_weight_kg.toLocaleString()} ${tc("units.kg")}` : "\u2014"} />
      </div>

      {/* Status Actions */}
      {container.status !== "open" && container.status !== "delivered" && (
        <div className="bg-white rounded-lg border p-4">
          <h3 className="text-sm font-semibold text-gray-700 mb-3">{t("container.statusActions")}</h3>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setSealForm((f) => ({ ...f, seal_number: e.target.value }))}
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setSealForm((f) => ({ ...f, temp_setpoint_c: e.target.value }))}
                    className="w-full border rounded px-3 py-2 text-sm"
                    placeholder="-1.5"
                  />
                </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setExportForm((f) => ({ ...f, vessel_name: e.target.value }))}
                    className="w-full border rounded px-3 py-2 text-sm"
                  />
                </div>
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setExportForm((f) => ({ ...f, voyage_number: e.target.value }))}
                    className="w-full border rounded px-3 py-2 text-sm"
                  />
                </div>
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => setExportForm((f) => ({ ...f, shipping_line_id: e.target.value }))}
                    className="w-full border rounded px-3 py-2 text-sm"
                  >
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setExportForm((f) => ({ ...f, eta: e.target.value }))}
                    className="w-full border rounded px-3 py-2 text-sm"
                  />
                </div>
              </div>
      ...
      <div className="bg-white rounded-lg border p-4">
        <div className="flex items-center justify-between text-xs text-gray-500 mb-1">
          ...
          <span>{container.pallet_count} / {container.capacity_pallets} {tc("units.pallets")}</span>
        </div>
      ...
      <div className="bg-white rounded-lg border p-4">
        ...
        {editing ? (
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="block text-xs text-gray-500 mb-1">{t("container.shippingLine")}</label>
                <select
                  value={editForm.shipping_line_id}
                  onChange={(e) => setEditForm((f) => ({ ...f, shipping_line_id: e.target.value }))}
              ...
              <div>
                <label className="block text-xs text-gray-500 mb-1">{t("container.vesselName")}</label>
                <input
                  value={editForm.vessel_name}
                  onChange={(e) => setEditForm((f) => ({ ...f, vessel_name: e.target.value }))}
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="block text-xs text-gray-500 mb-1">{t("container.voyageNumber")}</label>
                <input
                  value={editForm.voyage_number}
                  onChange={(e) => setEditForm((f) => ({ ...f, voyage_number: e.target.value }))}
                  disabled={container.locked_fields?.includes("voyage_number")}
                  className="w-full border rounded px-3 py-2 text-sm disabled:bg-gray-100 disabled:cursor-not-allowed"
                />
              </div>
              <div>
                <label className="block text-xs text-gray-500 mb-1">{t("container.etaLabel")}</label>
                <input
                  type="date"
                  value={editForm.eta}
          ...
          <div className="grid grid-cols-2 gap-y-2 text-sm">
            ...
            <Row label={t("container.shippingLine")} value={container.shipping_line_name || "\u2014"} />
            <Row label={t("container.vesselName")} value={container.vessel_name || "\u2014"} />
            <Row label={t("container.voyageNumber")} value={container.voyage_number || "\u2014"} />
            <Row label={t("container.etaLabel")} value={container.eta ? new Date(container.eta).toLocaleDateString() : "\u2014"} />
          </div>
        )}
      </div>

      {/* Timestamps */}
      {(container.sealed_at || container.dispatched_at || container.arrived_at || container.delivered_at) && (
        <div className="bg-white rounded-lg border p-4">
          <h3 className="text-sm font-semibold text-gray-700 mb-3">{t("container.timestamps")}</h3>
          <div className="grid grid-cols-2 gap-y-2 text-sm">
            {container.sealed_at && (
              <>
                <Row
                    ...
                    <div className="space-y-1">
                      {tp.lots.map((lot, i) => (
                        <p key={i} className="text-xs text-gray-700">
                          ...
                          {" \u2014 "}
                          {lot.grade || "?"} / {lot.size || "?"}{lot.box_size_name ? ` \u00b7 ${lot.box_size_name}` : ""} ({lot.box_count} {tc("units.boxes")})
                        </p>
                      ))}
                    </div>
                  </div>
          ...
          <div className="bg-white rounded-lg shadow-xl w-full max-w-2xl mx-4 max-h-[80vh] flex flex-col">
            ...
            <div className="px-5 py-4 border-t space-y-3">
              ...
              <div className="flex items-center justify-end gap-3">
                <button
                  onClick={() => { setShowLoadModal(false); setForceOverride(false); }}
                  className="px-4 py-2 text-sm font-medium text-gray-700 bg-gray-100 rounded-md hover:bg-gray-200 transition-colors"
                >
                  {tc("actions.cancel")}
                </button>
      ...
      {showDeleteConfirm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="bg-white rounded-lg shadow-xl w-full max-w-sm mx-4 p-6">
            <h3 className="text-lg font-semibold text-gray-800 mb-2">{t("detail.deleteTitle")}</h3>
